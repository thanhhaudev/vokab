...
final class AntigravityQuotaServiceTests: XCTestCase {
    ...
    func test_usesDetectedServer_whenRunning() async throws {
        ...
        let svc = AntigravityQuotaService(
            ...
            fetch: { _, _ in fetchCount.increment(); return self.sampleSummary(at: clock.now()) },
    ...
    func test_spawns_whenNoServerDetected() async throws {
        let clock = ClockBox(Date(timeIntervalSince1970: 1000))
        let spawned = CounterBox()
        let svc = AntigravityQuotaService(
            detect: { nil },
            fetch: { _, _ in XCTFail("no detected server"); throw AntigravityQuotaError.transient },
            spawnFetch: { spawned.increment(); return self.sampleSummary(at: clock.now()) },
            now: { clock.now() })
        _ = await svc.summary()
        XCTAssertEqual(spawned.value, 1)
    }
...
/// Thread-safe call counter for @Sendable test closures (mutating a captured
/// `var` from a concurrent closure is a Swift 6 error).
